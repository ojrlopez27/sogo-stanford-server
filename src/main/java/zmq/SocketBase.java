...
public abstract class SocketBase extends Own implements IPollEvents, Pipe.IPipeEvents
{
    ...
    public final int getSocketOpt(int option) throws Exception //ojrlopez
    {
        if (ctxTerminated) {
            errno.set(ZError.ETERM);
            return -1;
    ...
    private boolean processCommands(int timeout, boolean throttle) throws Exception //ojrlopez
    {
        Command cmd;
        if (timeout != 0) {
